...
try:
    ...
    s3_output_path = "data-lake-demo/bronze/dbRooms"
    ...
    # Reemplazar valores nulos y transformar el esquema para ser compatible con Parquet
    for column, dtype in df.dtypes:
        if dtype == 'null':
            df = df.drop(column)
        elif dtype.startswith('struct'):
            # Manejo de columnas de tipo struct (se pueden mantener tal cual o procesar según necesidad)
            continue
        elif 'string' in dtype:
            df = df.na.fill({column: ''})
        else:
